...
class FVM:
    ...
    def set_inputs(self, pos_source, pos_rec, abs_coeff):
        ...
        self.Ws = 0.01 #Source point power [Watts] interrupted after "sourceon_time" seconds; 10^-2 W => correspondent to 100dB

        self.sourceon_time =  1.5 #time that the source is ON before interrupting [s]
        self.recording_time = 4.2 #total time recorded for the calculation [s]
        ...
        self.center_freq = self.fc_low * np.power(2,((np.arange(0, self.x_frequencies+1) / self.num_octave)))
    ...
    def init_gmsh(self):
        ...
        self.mesh = gmsh.open(self.file) #open the file
        ...
        self.nodecoords = coords.reshape((-1,3)) #coordinates reshaped in a matrix 3xnumber of nodes
        ...
        for elem in range(len(voluEl)):
            self.velement = self.velement + 1 #scalar number of the volume elements
        ...
        for i in volumeEl_dict.keys():
            ...
            centre_cell[i] = []
            #print(i)
            vc0 = self.nodecoords[node_indices[volumeEl_dict[i][0]],:] #coordinates of node 0
            #vc0 = gmsh.model.mesh.getNode(volumeEl_dict[i][0])[0] #Coordinates of the node number zero of the volume element i
            #print(nc0)
            vc1 = self.nodecoords[node_indices[volumeEl_dict[i][1]],:]
            vc2 = self.nodecoords[node_indices[volumeEl_dict[i][2]],:]
            vc3 = self.nodecoords[node_indices[volumeEl_dict[i][3]],:]
        ...
        self.cell_center = np.array(()) #initialization of an array of centre cell coordinates from the centre_cell dictionary
        for key in centre_cell:
            self.cell_center = np.append(self.cell_center,centre_cell[key])
        self.cell_center = self.cell_center.reshape((-1,3))

        self.cell_volume = np.array(()) #initialization of an array of cell volumes from the vcell_dict dictionary
        for key in vcell_dict:
            self.cell_volume = np.append(self.cell_volume,vcell_dict[key])
        ...
        for i in boundaryEl_dict.keys():
            ...
            centre_area[i] = []
            #print(i)
            bc0 = self.nodecoords[node_indices[boundaryEl_dict[i][0]],:]
            ...
            #print(nc0)
            bc1 = self.nodecoords[node_indices[boundaryEl_dict[i][1]],:]
            #gmsh.model.mesh.getNode(boundaryEl_dict[i][1])[0] #Coordinates of the node number one of the volume element i
            #print(nc1)
            bc2 = self.nodecoords[node_indices[boundaryEl_dict[i][2]],:]
        ...
        def abs_term(th,abscoeff_list):
            ...
            for abs_coeff in abscoeff_list:
                print(abs_coeff)
                if th == 1:
                    Absx = (self.c0*abs_coeff)/4 #Sabine
                elif th == 2:
                    Absx = (self.c0*(-log(1-abs_coeff)))/4 #Eyring
                elif th == 3:
                    Absx = (self.c0*abs_coeff)/(2*(2-abs_coeff)) #Modified by Xiang
        ...
        self.interior_tet = np.zeros((self.velement, self.velement)) #initialization matrix of tetrahedron per tetrahedron

        for i in range(self.velement): #for each tetrahedron, take its centre
            print(i)
            self.cell_center_i = self.cell_center[i]
            for j in range(self.velement): #for each tetrahedron, take its centre
                ...
                if i != j: #if the tetrahedrons are not the same one, then check if there are shared nodes in between the two tetrahedron i and j
                    ...
                    if len(shared_nodes) == 3: #after have done this for all the nodes, if the cound is 3 then calculate the shared area between the tetrahedrons
                        sc0 = self.nodecoords[node_indices[shared_nodes[0]],:]
                        #sc0 = gmsh.model.mesh.getNode(shared_nodes[0])[0] #coordinates of node 0
                        sc1 = self.nodecoords[node_indices[shared_nodes[1]],:]
                        #sc1 = gmsh.model.mesh.getNode(shared_nodes[1])[0] #coordinates of node 1
                        sc2 = self.nodecoords[node_indices[shared_nodes[2]],:]
                        ...
                        shared_distance = np.linalg.norm(self.cell_center_i - self.cell_center[j])
                        ...
                        self.interior_tet[i, j] = shared_area/shared_distance #division between shared area and shared distance
                    else:
                        shared_area = 0
                        self.interior_tet[i, j] = shared_area

        self.interior_tet_sum = np.sum(self.interior_tet, axis=1) #sum of interior_tet per columns (so per i element)
        ...
        for group in vGroupsNames:
            ...
            Abs_term = abs_term(self.th, abscoeff_list) #calculates the absorption term based on the type of boundary condition th
            for entity in entities:
                self.absorption_coefficient[entity] = abscoeff_list
        ...
        for entity, Abs_term in surface_absorption:   
            ...
            for i in range(0, len(face_nodes_per_entity), 3): # per each element basically, goes trhough the nodes of each face 3by3
                ...
                fc0 = self.nodecoords[node_indices[f[0]],:]
                #fc0 = gmsh.model.mesh.getNode(f[0])[0] #coordinates of vertix 0
                fc1 = self.nodecoords[node_indices[f[1]],:]
                fc2 = self.nodecoords[node_indices[f[2]],:]
                ...
                self.surface_areas[entity] = surf_area_tot
        #######################################################################################################
                    ...
                    for surface_idx, surface in enumerate(bounNode): #for index and surface in the number of nodes
                        ...
                        if nodes_set == surface_set: #if these are equal, it means that the tetrahedron into consideration has a surface in the boundary and therefore is_boundary gets the value of True.
                            ...
                            if is_boundary: #if the surface is at the boundary, then take the coordinates of each vertix
                                ...
                                bc0 = self.nodecoords[node_indices[nodes[0]],:]
                                bc1 = self.nodecoords[node_indices[nodes[1]],:]
                                bc2 = self.nodecoords[node_indices[nodes[2]],:]
                                ...
                                self.total_boundArea += face_area #add to the total boundary area
    ...
    def calculate(self):
        ...
        #Fixed inputs
        self.pRef = 2 * (10**-5) #Reference pressure in Pa
        self.rho = 1.21 #air density [kg.m^-3] at 20°C

        self.V = sum(self.cell_volume)
        self.S = self.total_boundArea #surface area of the room

        #Time resolution
        self.t = np.arange(0, self.recording_time, self.dt) #mesh point in time
        self.recording_steps = ceil(self.recording_time/self.dt) #number of time steps to consider in the calculation

        sum_alpha_average = 0
        self.Eq_A = 0
        #Absorption parameters for room
        for entity in self.surface_areas:
            print(entity)
            sum_alpha_average += np.multiply(self.absorption_coefficient[entity], self.surface_areas[entity])
            self.Eq_A += np.multiply(self.absorption_coefficient[entity], self.surface_areas[entity])
        alpha_average = sum_alpha_average/self.S #average absorption

        #Diffusion parameters
        mean_free_path = (4*self.V)/self.S #mean free path for 3D
        mean_free_time= mean_free_path/self.c0 #mean free time for 3D
        mean_free_time_step = int(mean_free_time/self.dt)
        Dx = (mean_free_path*self.c0)/3 #diffusion coefficient for proportionate rooms x direction
        Dy = (mean_free_path*self.c0)/3 #diffusion coefficient for proportionate rooms y direction
        Dz = (mean_free_path*self.c0)/3 #diffusion coefficient for proportionate rooms z direction
        ...
        #VOLUME ORIGINAL
        Vs = self.cell_volume[source_idx] #volume of the source = to volume of cells where the volume is 
        # Vs = 1
        ...
        w1 = self.Ws / Vs #w1 = round(Ws/Vs,4) #power density of the source [Watts/(m^3))]
        sourceon_steps = ceil(self.sourceon_time/self.dt) #time steps at which the source is calculated/considered in the calculation
        s1 = np.multiply(w1,np.ones(sourceon_steps)) #energy density of source number 1 at each time step position
        source1 = np.append(s1, np.zeros(self.recording_steps-sourceon_steps)) #This would be equal to s1 if and only if recoding_steps = sourceon_steps
        ...
        s = np.zeros((self.velement)) #matrix of zeros for source
        ...
        x_coordinates = self.nodecoords[:, 0]
        ...
        y_coordinates = self.nodecoords[:, 1]
        ...
        ###############################################################################
        self.line_rec_x_idx_list = []
        self.dist_x = np.array([])
        ...
        ###############################################################################
        self.line_rec_y_idx_list = []
        self.dist_y = np.array([])
        ...
        for iBand in range(self.nBands):
            freq = self.center_freq[iBand]    
            
            w_new = np.zeros(self.velement) #unknown w at new time level (n+1)
            ...
            w_rec = np.zeros(self.recording_steps) #energy density at the receiver
            
            #Computing w;
            for steps in range(0, self.recording_steps):
                #Compute w at inner mesh points
                time_steps = steps * self.dt #total time for the calculation
                
                #Computing w_new (w at n+1 time step)
                            
                w_new = np.divide((np.multiply(w_old, (1 - beta_zero_freq[iBand]))), (1 + beta_zero_freq[iBand])) - \
                    np.divide((2 * self.dt * self.c0 * self.m_atm * w), (1 + beta_zero_freq[iBand])) + \
                        np.divide(np.divide((2 * self.dt * Dx * (self.interior_tet@w)), self.cell_volume), (1 + beta_zero_freq[iBand])) + \
                            np.divide((2 * self.dt * s), (1 + beta_zero_freq[iBand])) #The absorption term is part of beta_zero
                ...
                if steps == round(1*mean_free_time_step + sourceon_steps + (self.dist_sr / self.c0)):
                    w_1l = w_new
                    
                if steps == round(2*mean_free_time_step + sourceon_steps + (self.dist_sr/self.c0)):
                    w_2l = w_new
                    
                if steps == round(3*mean_free_time_step + sourceon_steps + (self.dist_sr/self.c0)):
                    w_3l = w_new
            
                if steps == round(5*mean_free_time_step + sourceon_steps + (self.dist_sr/self.c0)):
                    w_5l = w_new
                ...
                if self.tcalc == "stationarysource":
                    #ORIGINAL
                    #s[source_idx] = source1[0]
            ...
            self.w_new_band.append(w_new)
            self.w_rec_band.append(w_rec)
    ...
    def collect_results(self):
        ...
        for iBand in range(self.nBands):
            ...
            for xr in self.line_rec_x_idx_list:
                w_rec_x = self.w_new_band[iBand][xr]
                w_rec_x_end = np.append(w_rec_x_end, w_rec_x)
                
            w_rec_y_end = np.array([])
            for yr in self.line_rec_y_idx_list:
                w_rec_y = self.w_new_band[iBand][yr]
                w_rec_y_end = np.append(w_rec_y_end, w_rec_y)

            spl_stat_x = 10*np.log10(self.rho * self.c0 * (((self.Ws) / (4 * math.pi * (self.dist_x**2))) + ((abs(w_rec_x_end) * self.c0)))/(self.pRef**2))
            spl_stat_y = 10*np.log10(self.rho*self.c0*(((self.Ws)/(4*math.pi*(self.dist_y**2))) + ((abs(w_rec_y_end)*self.c0)))/(self.pRef**2)) #It should be the spl stationary 

            press_r = ((abs(self.w_rec_band[iBand]))*self.rho*(self.c0**2)) #pressure at the receiver
            spl_r = 10*np.log10(((abs(self.w_rec_band[iBand]))*self.rho*(self.c0**2))/(self.pRef**2)) #,where=press_r>0, sound pressure level at the receiver
            spl_r_norm = 10*np.log10((((abs(self.w_rec_band[iBand]))*self.rho*(self.c0**2))/(self.pRef**2)) / np.max(((abs(self.w_rec_band[iBand]))*self.rho*(self.c0**2))/(self.pRef**2))) #normalised to maximum to 0dB
            spl_r_tot = 10*np.log10(self.rho*self.c0*((self.Ws/(4*math.pi*self.dist_sr**2))*np.exp(-self.m_atm*self.dist_sr) + ((abs(self.w_rec_band[iBand]))*self.c0)/(self.pRef**2))) #spl total (including direct field) at the receiver position????? but it will need to be calculated for a stationary source 100dB
            
            #Find the energy decay part of the overal calculation
            idx_w_rec = np.where(self.t == self.sourceon_time)[0][0] #index at which the t array is equal to the sourceon_time; I want the RT to calculate from when the source stops.
            w_rec_off = self.w_rec_band[iBand][idx_w_rec:] #cutting the energy density array at the receiver from the idx_w_rec to the end
            ...
            if self.tcalc == "decay":
                t60 = t60_decay(self.t, sch_db, idx_w_rec) #called function for calculation of t60 [s]
                edt = edt_decay(self.t, sch_db, idx_w_rec) #called function for calculation of edt [s]
                #Eq_A = 0.16*V/t60 #equivalent absorption area defined from the RT 
                c80 = clarity(t60, self.V, self.Eq_A[iBand], self.S, self.c0, self.dist_sr) #called function for calculation of c80 [dB]
                d50 = definition(t60, self.V, self.Eq_A[iBand], self.S, self.c0, self.dist_sr) #called function for calculation of d50 [%]
                ts = centretime(t60, self.Eq_A[iBand], self.S) #called function for calculation of ts [ms]
